...
* Move `docs/bot.conf.example` to `~/.niblet/bot.conf` and fill in the config options you care about. Ones blank or removed will disable relevant functionality.
* Run `./nibletd -a` to add a server.
* Run `./nibletd` to start the bot or `./nibletd -c /path/to/bot.conf` for non-standard config location (outside of `~/.niblet`).
* Run `./nibletctl command master-password` to get the master admin password (needed to add regular admin accounts)
* Join `#niblet` on a server with the bot (or invite it to another channel)
...
Below is an OpenSSL command example that will create a `niblet-cert.pem` and `niblet-key.pem` with `10y` validity (self-signed):
> openssl req -x509 -nodes -sha512 -newkey rsa:4096 -keyout niblet-key.pem -out niblet-cert.pem -days 3650 -subj "/CN=YourBotNick"

### Configure SASL

Configure the bot to use SASL to authenticate (usually used for `NickServ` identification)

`EXTERNAL` usually mean client TLS certificate authentication; `USERPASS` is a Niblet-specific term that selects the strongest user/password algorithm.
...
> /msg &lt;bot> config channel:#niblet &lt;setting> &lt;value>
